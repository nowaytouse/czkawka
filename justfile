...
_clippy_all_features toolchain="":
    #!/usr/bin/env bash
    set -euo pipefail
    if [[ "$(uname -s)" == "Darwin" ]]; then
        cargo {{toolchain}} clippy --fix --allow-dirty --allow-staged --workspace --all-features --all-targets --exclude krokiet --exclude cedinia
        cargo {{toolchain}} clippy --fix --allow-dirty --allow-staged -p krokiet -p cedinia --no-default-features --features "{{macos_gui_clippy_features}}" --all-targets
    else
        cargo {{toolchain}} clippy --fix --allow-dirty --allow-staged --all-features --all-targets
    fi
...
fix:
    grep -rl --null -F --include='*.rs' --include='*.slint' --include='*.md' --include='*.ftl' --exclude='AGENTS.md' --exclude='justfile' --exclude-dir='.git' --exclude-dir='target' -e '─' -e '–' -e '—' -e '…' . | xargs -0 perl -CSD -i -pe 's/[\x{2500}\x{2013}\x{2014}]/-/g; s/\x{2026}\.?/.../g' || true
...
fixn:
    cargo +nightly fmt
    just _clippy_all_features '+nightly'
    cargo +nightly fmt
