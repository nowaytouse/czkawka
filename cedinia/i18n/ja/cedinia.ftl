...
notifications_disabled_banner = 通知を無効にしました
...
settings_common_label = 一般的な設定
settings_excluded_items = 除外アイテム（グローブパターン、カンマ区切り）
settings_excluded_items_placeholder = 例: *.tmp, */.git/*, */node_modules/*
settings_allowed_extensions = 許可された拡張 (空 = すべて)
...
settings_hash_size_desc = サイズが大きいほど、誤検出が少ないが、似たような画像も見つかりません
settings_hash_alg = ハッシュアルゴリズム
settings_image_filter = フィルターのサイズ変更
settings_ignore_same_size = 同じサイズの画像を無視
settings_gallery_image_fit_cover = ギャラリー: 正方形にクロップ
settings_gallery_image_fit_cover_desc = タイルを塗りつぶす; 元のアスペクト比を維持するには無効にする
settings_big_files_header = 最大のファイル
settings_search_mode = 検索モード
settings_file_count = ファイル数
settings_same_music_header = 音楽用ダプリケート
...
diag_confirm_clear_thumbnails = サムネイルキャッシュをすべて消去しますか？
...
status_deleted_selected = 削除された選択
status_deleted_with_errors = 削除されたエラーがあります
...
stage_scanning_size = サイズでスキャン中
stage_pre_hash = 事前ハッシュ
stage_full_hash = ハッシュ
stage_loading_cache = キャッシュを読み込み中
stage_saving_cache = キャッシュを保存中
stage_calculating_image_hashes = 画像ハッシュを計算中
stage_comparing_images = 画像の比較
stage_calculating_video_hashes = ビデオハッシュの計算
stage_checking_files = ファイルを確認中
stage_checking_extensions = エクステンションを確認中
stage_checking_names = 名前を確認中
stage_reading_music_tags = 音楽タグを読み込み中
stage_comparing_tags = タグの比較
stage_calculating_music_fingerprints = 音楽のフィンガープリントの計算
stage_comparing_fingerprints = 指紋の比較
stage_extracting_exif = EXIF タグを読み込み中
stage_creating_video_thumbnails = 動画サムネイルを作成しています
stage_processing_videos = ビデオを処理中
stage_deleting = ファイルを削除中
stage_renaming = ファイル名の変更
stage_moving = ファイルを移動中
stage_hardlinking = ハードリンクの作成
...
path_edit_title_include = Include に追加
...
path_edit_no_newlines = パスに改行を含めることはできません — 入力キーは許可されていません
ctx_menu_title = 開く
