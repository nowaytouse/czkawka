...
- **`run_gui.py` freshness check** – the launcher also watches `<crate>/ui/`, `Cargo.toml`, `build.rs`, and workspace `Cargo.lock`; a full workspace dependency bump still requires a manual rebuild if the binary timestamp is unchanged.
...
- **Simplified Chinese blank regions [FIXED in fork]** – Krokiet bundles `Noto Sans SC` via Slint `import` in `main_window.slint` (`krokiet/ui/fonts/NotoSansSC-Regular.otf`, OFL). Missing translation keys still fall back to English strings from i18n-embed; they are not blank. Hand-editing non-English `.ftl` files remains forbidden (Crowdin overwrites them).
