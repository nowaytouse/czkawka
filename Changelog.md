...
- **Improved Similar Images Filter**: Enhanced "Only show same size" to find same-sized images regardless of similarity slider settings. Maintained standard BK-Tree search architecture while internally overriding tolerance to the maximum possible value for this mode.
- **Improved Mutual Exclusivity and Visual Feedback**: Toggling "Only same size" now automatically disables and unchecks "Ignore same size" and "Size Ratio Filter". Additionally, the similarity slider now visually slides to its maximum position and becomes greyed out, restoring its original value when unchecked.
