...
- **Improved Similar Images Filter**: Enhanced "Only show same size" to find same-sized images regardless of similarity slider settings. Maintained standard BK-Tree search architecture while internally overriding tolerance for this mode.
- **Implemented Mutual Exclusivity and Visual Feedback**: Toggling "Only same size" now automatically disables and unchecks "Ignore same size" and "Size Ratio Filter", and visually disables the similarity slider to prevent conflicting configurations.
