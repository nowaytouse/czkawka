...
rust_checking_empty_files_content = 正在檢查 { $items_stats } 個文件的內容（{ $size_stats }）
...
rust_hiding_links = 正在隱藏硬鏈接 { $items_stats }
...
rust_trash_confirmation = 確定要將所選項目移到回收站嗎？
rust_trash_confirmation_unsupported_volumes = 注意：並非所有卷都支持移到回收站。不支持的捲上此操作將始終失敗.
rust_trash_confirmation_number_simple = 已選中 { $items } 項.
rust_trash_confirmation_number_groups = 已在 { $groups } 個組中選中 { $items } 項.
rust_trash_confirmation_selected_all_in_group = 已在 { $groups } 個組中全選.
...
selection_all_except_biggest_size = 全選除最大大小外
selection_all_except_smallest_size = 全選除最小大小外
selection_all_except_biggest_resolution = 全選除最大分辨率外
selection_all_except_smallest_resolution = 全選除最小分辨率外
selection_all_except_newest = 全選除最新外
selection_all_except_oldest = 全選除最舊外
selection_all_except_longest_path = 全選除最長路徑外
selection_all_except_shortest_path = 全選除最短路徑外
selection_all_except_highest_quality = 全選除最高畫質外
...
subsettings_images_size_ratio = 尺寸比例篩選
subsettings_images_max_size_ratio = 最大尺寸比例
subsettings_images_only_same_size_hint = 啓用後，結果僅顯示文件大小完全相同的圖片
subsettings_images_ignore_same_resolution = 忽略分辨率相同的圖片
...
subsettings_videos_ignore_same_resolution = 忽略分辨率相同的視頻
subsettings_videos_audio_check_content = 按音頻指紋比較
subsettings_videos_audio_preset = 快速預設
subsettings_videos_audio_preset_custom = 自定義
subsettings_videos_audio_preset_identical = 相同視頻
subsettings_videos_audio_preset_clip = 較長視頻中的片段
subsettings_videos_audio_preset_similar = 相似內容
subsettings_videos_audio_similarity_percent = 相似度 [%]
subsettings_videos_audio_length_ratio = 最小長度比（較短/較長）
subsettings_videos_audio_min_duration_seconds = 最短文件時長 [秒]
...
subsettings_empty_files_type = 要查找的其他文件類型
subsettings_empty_files_zero_byte_content = 僅含空字節的文件
subsettings_empty_files_zero_byte_content_hint = 同時查找非空但內容全爲 0x00 空字節的文件.
subsettings_empty_files_non_printable_content = 僅含不可打印字符的文件
subsettings_empty_files_non_printable_content_hint = 同時查找非空但僅含不可打印 ASCII 字符的文件：空字符、空格、製表符、回車、換行、垂直製表符、換頁符.
...
subsettings_broken_files_video_ffprobe = 視頻（ffprobe）
subsettings_broken_files_video_ffprobe_info = 使用 ffprobe 快速檢查視頻（頭信息校驗）.
subsettings_broken_files_video_ffmpeg = 視頻（ffmpeg）
subsettings_broken_files_video_ffmpeg_info = 使用 ffmpeg 深度檢查視頻（完整解碼）。非常慢，且可能報告吹毛求疵的錯誤，即使文件可正常播放.
subsettings_broken_files_font = 字體
subsettings_broken_files_markup = 標記（JSON/XML/TOML）
...
settings_select_header_text = 選擇彈窗選項
...
settings_select_label_one_smallest_text = 保留最小一項
settings_select_label_one_biggest_text = 保留最大一項
settings_select_label_except_smallest_text = 除最小外
settings_select_label_except_biggest_text = 除最大外
settings_select_label_one_oldest_text = 保留最舊一項
settings_select_label_one_newest_text = 保留最新一項
settings_select_label_except_oldest_text = 除最舊外
...
settings_select_label_except_longest_text = 除最長外
...
subsettings_video_optimizer_noise_reduction_strength = 降噪強度
subsettings_video_optimizer_use_custom_command = 使用自定義 FFmpeg 命令
subsettings_video_optimizer_custom_command_hint = 命令中使用 {"{PATH}"} 表示輸入文件。輸出路徑會自動追加.
subsettings_video_optimizer_hardware_encoder = 硬件編碼器
...
subsettings_temporary_files_extensions_text = 擴展名：
subsettings_temporary_files_extensions_hint_text = 以逗號分隔的臨時文件擴展名/後綴（例如 .tmp,.bak,~）。重置可恢復內置默認值.
...
optimize_crf_hint = 數值越低畫質越好。0 接近無損，51 最差。建議：18–28.
optimize_noise_reduction_hint = 降噪可能顯著增加編碼時間.
optimize_noise_reduction_strength_hint = 1 = 最輕，10 = 最強降噪.
...
popup_custom_save_restore_text = 保存並恢復篩選數據
