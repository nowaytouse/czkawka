...
- **"Choose application" dialog right after scan [FIXED]** – macOS opens this when `open::that` runs on a result path. That only happens from `row_open_item_with_index` (double-click, context menu, or **Return** in `selectable_tree_view.slint`). Return key-up from activating **Scan** could reach the list while `last_selected_idx` was still set from an earlier row click; `scan_ended` now resets list keyboard state before `scanning = false`, and scan start clears it too. Return/double-click also skip `header_row` rows.
- **Double-click opens the file with the default app [BY DESIGN]** – routes to `open::that(path)` in `connect_row_selection.rs` (same as upstream). If the OS has no default app for that type, macOS shows "Choose application"; that is not a second Krokiet instance.
